...
class HistoryItemDetailScreen extends ConsumerWidget {
  ...
  const HistoryItemDetailScreen(
      {super.key, required this.workspaceId, required this.historyId});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final colors = Theme.of(context).extension<AppColorsExtension>()!;
    final itemAsync = ref.watch(
        historyItemProvider((workspaceId: workspaceId, historyId: historyId)));

    return itemAsync.when(
      loading: () => const LoadingView(type: LoadingViewType.detail),
      error: (err, _) => ErrorView(
          message: err.toString(),
          onRetry: () => ref.invalidate(historyItemProvider(
              (workspaceId: workspaceId, historyId: historyId)))),
      data: (item) {
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text('PUBLISHED POST #$historyId',
                style:
                    AppTextStyles.heading2.copyWith(color: colors.textPrimary)),
          ),
          body: PullToRefresh(
            onRefresh: () async => ref.invalidate(historyItemProvider(
                (workspaceId: workspaceId, historyId: historyId))),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: SelectableText(item.finalText,
                        style: AppTextStyles.bodyLg
                            .copyWith(color: colors.textPrimary)),
                  ),
                  const SizedBox(height: 24.0),
                  _row('Source Channel', item.sourceChannel ?? '—', colors),
                  _row('Telegram Message ID', item.telegramMessageId ?? '—',
                      colors),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value,
              style: AppTextStyles.mono.copyWith(color: colors.textSecondary)),
        ],
      ),
    );
  }
