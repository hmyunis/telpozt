...
class QueueRepository {
  ...
  Future<List<QueueItem>> fetchQueue(int workspaceId) async {
    ...
    final response = await client.get('/workspaces/$workspaceId/queue',
  ...
  Future<void> createQueueItem(int workspaceId, String rawText) async {
    final client = ref.read(apiClientProvider);
    await client
        .post('/workspaces/$workspaceId/queue', data: {'raw_text': rawText});
  }

  Future<void> updateItemState(int workspaceId,
  ...
  Future<void> saveManualText(int workspaceId,
      {required int queueId, required String generatedText}) async {
    final client = ref.read(apiClientProvider);
    await client.patch('/workspaces/$workspaceId/queue/$queueId/text',
  ...
  Future<QueueItem> fetchQueueItem(int workspaceId, int queueId) async {
    final client = ref.read(apiClientProvider);
    final response =
        await client.get('/workspaces/$workspaceId/queue/$queueId');
    return QueueItem.fromJson(response.data['data'] as Map<String, dynamic>);
  }

  Future<Map<String, String>> fetchPromptDetails(
      int workspaceId, int queueId) async {
    final client = ref.read(apiClientProvider);
    final response =
        await client.get('/workspaces/$workspaceId/queue/$queueId/prompt');
...
final queueRepositoryProvider =
    Provider<QueueRepository>((ref) => QueueRepository(ref));

final queueProvider =
    FutureProvider.family<List<QueueItem>, int>((ref, workspaceId) {
  return ref.read(queueRepositoryProvider).fetchQueue(workspaceId);
});

final queueItemProvider =
    FutureProvider.family<QueueItem, ({int workspaceId, int queueId})>(
        (ref, args) {
  return ref
      .read(queueRepositoryProvider)
      .fetchQueueItem(args.workspaceId, args.queueId);
});

final queuePromptProvider = FutureProvider.family<Map<String, String>,
    ({int workspaceId, int queueId})>((ref, args) {
  return ref
      .read(queueRepositoryProvider)
      .fetchPromptDetails(args.workspaceId, args.queueId);
});
